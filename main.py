...
# =========================
RU_TO_LATIN_PAIRS = {
    # Частые пары, которые уже встречались в сигналах. Список можно расширять без миграций БД.
    "макей": "mackay",
    "метеоретс": "meteorettes",
    "метеореттес": "meteorettes",
    "саутерн": "southern",
    "дистриктс": "districts",
    "спартанс": "spartans",
    "фэйз": "faze",
...
def soft_latinize(text: str) -> str:
    text = normalize_text(text)
    for ru, en in RU_TO_LATIN_PAIRS.items():
        text = re.sub(rf"\b{ru}\b", en, text)
    return text
...
def text_similarity(a: str, b: str) -> float:
    ...
    a_tokens = {x for x in a.split() if len(x) >= 3}
    b_tokens = {x for x in b.split() if len(x) >= 3}
    token_score = 0.0
...
def team_similarity(match_a: str, match_b: str) -> float:
    a1, a2 = split_teams(match_a)
    b1, b2 = split_teams(match_b)

    if not a1 or not a2 or not b1 or not b2:
        return text_similarity(match_a, match_b)

    direct = (text_similarity(a1, b1) + text_similarity(a2, b2)) / 2
    reverse = (text_similarity(a1, b2) + text_similarity(a2, b1)) / 2
...
def get_pending_similar_bets(parsed: dict):
    ...
    # 1) Надёжный быстрый путь: точный event_key.
    ...
    # 2) Fallback для разных написаний одного матча.
    # Не сравниваем все матчи в то же время: обязательно нужна похожесть команд.
    sport = parsed.get("sport")
    match_start_at = parsed.get("match_start_at")
    if not sport or not match_start_at:
        db.close()
        return []
    ...
    time_from = (dt - timedelta(minutes=5)).isoformat()
    time_to = (dt + timedelta(minutes=5)).isoformat()

    candidate_rows = db.execute(
        """
        SELECT id, sport, match_name, market, odds, stake, bookmaker, result_status,
               selection_side, market_type, market_side, line_value, event_key,
               semantic_key, market_group, tournament, match_start_at
        FROM bets
        ...
        LIMIT 50
        """,
        (sport, time_from, time_to),
    ).fetchall()
    db.close()

    result = []
    for row in candidate_rows:
        existing_match_name = row[2] or ""
        existing_tournament = row[15] or ""

        teams_score = team_similarity(parsed.get("match_name", ""), existing_match_name)
        tourn_score = tournament_similarity(parsed.get("tournament", ""), existing_tournament)

        # Основной фильтр — команды. Турнир только усиливает пограничные случаи.
        if teams_score >= 0.62 or (teams_score >= 0.52 and tourn_score >= 0.45):
...
def add_count_bar_chart(sheet, title: str, data_start_row: int = 1, position: str = "K2"):
    """Добавляет bar chart по колонкам A=категория, B=count."""
    try:
        last = data_start_row
        while last + 1 <= sheet.max_row and sheet.cell(row=last + 1, column=1).value:
            last += 1
        ...
        data = Reference(sheet, min_col=2, min_row=data_start_row, max_row=last)
        cats = Reference(sheet, min_col=1, min_row=data_start_row + 1, max_row=last)
        ...
        logger.warning(f"Excel aggregate chart failed: {e}")


def add_aggregate_sheet(wb: Workbook, title: str, headers: list[str], rows: list[tuple], chart_title: str):
    ws = wb.create_sheet(title)
    ws.append(headers)
    for row in rows:
        ws.append(list(row))
    style_sheet(ws)
    add_count_bar_chart(ws, chart_title, 1, "K2")
    return ws
...
def export_all_to_xlsx(user_id: int) -> str | None:
    ...
    market_all = db.execute("""
        SELECT COALESCE(market_group, 'Другое') AS group_name,
               COUNT(*) AS bets_count,
               ...
               COALESCE(SUM(CASE WHEN result_status='pending' THEN 1 ELSE 0 END), 0) AS pending_count
        FROM bets
        WHERE user_id = ?
        GROUP BY COALESCE(market_group, 'Другое')
    ...
    sport_all = db.execute("""
        SELECT COALESCE(sport, 'Unknown') AS group_name,
               COUNT(*) AS bets_count,
               COALESCE(SUM(stake), 0) AS total_stake,
               COALESCE(AVG(stake), 0) AS avg_stake,
               COALESCE(AVG(odds), 0) AS avg_odds,
    ...
    tournament_all = db.execute("""
        SELECT COALESCE(tournament, 'Unknown') AS group_name,
               COUNT(*) AS bets_count,
               COALESCE(SUM(stake), 0) AS total_stake,
               COALESCE(AVG(stake), 0) AS avg_stake,
               COALESCE(AVG(odds), 0) AS avg_odds,
        ...
        FROM bets
        WHERE user_id = ?
        GROUP BY COALESCE(tournament, 'Unknown')
        ORDER BY bets_count DESC
        LIMIT 50
    """, (user_id,)).fetchall()
    db.close()
    ...
    aggregate_headers = ["Group", "Count", "Total stake", "Avg stake", "Avg odds", "Avg EV", "Profit", "ROI %", "Pending"]
    add_aggregate_sheet(
        wb,
        "Markets_Analytics",
        aggregate_headers,
        [(g, c, round(ts, 2), round(avg_s, 2), round(avg_o, 2), round(avg_e, 2), round(p, 2), calc_roi(p, ts), pend) for g, c, ts, avg_s, avg_o, avg_e, p, pend in market_all],
        "Ставки по маркетам",
    )
    add_aggregate_sheet(
        wb,
        "Sports_Analytics",
        aggregate_headers,
        [(g, c, round(ts, 2), round(avg_s, 2), round(avg_o, 2), round(avg_e, 2), round(p, 2), calc_roi(p, ts), pend) for g, c, ts, avg_s, avg_o, avg_e, p, pend in sport_all],
        "Ставки по видам спорта",
    )
    add_aggregate_sheet(
        wb,
        "Tournaments_Analytics",
        aggregate_headers,
        [(g, c, round(ts, 2), round(avg_s, 2), round(avg_o, 2), round(avg_e, 2), round(p, 2), calc_roi(p, ts), pend) for g, c, ts, avg_s, avg_o, avg_e, p, pend in tournament_all],
        "Ставки по турнирам",
    )
