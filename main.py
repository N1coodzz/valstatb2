...
def risk_note_to_telegram(risk_status: str, risk_notes: str) -> str:
    ...
    title_map = {
        ...
        "opposite_no_value": "🟡 <b>ПЛЕЧО БЕЗ ПЛЮСА</b>",
    }
...
def risk_decision_kb():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="💾 Сохранить"), KeyboardButton(text="🚫 Отказаться")],
            [KeyboardButton(text="❌ Отмена")],
        ],
        resize_keyboard=True,
        is_persistent=True,
...
async def universal_message_handler(message: Message, state: FSMContext):
    ...
    # Если бот ждёт решение сохранить/отказаться, а пользователь пишет сумму без кнопки.
    if current_state == ShiftState.waiting_risk_decision.state:
        try:
            amount_probe = as_float(text)
            if amount_probe > 0:
                await message.answer(
                    "ℹ️ Сумму получил, но сначала нужно нажать <b>💾 Сохранить</b>.\n\n"
                    "После этого я попрошу сумму и занесу ставку в базу.",
                    reply_markup=risk_decision_kb(),
                )
                return
        except Exception:
            pass
        await message.answer("ℹ️ Выбери действие: сохранить ставку или отказаться.", reply_markup=risk_decision_kb())
        return

    # Если бот ждёт сумму — сохраняем только числовой ввод.
