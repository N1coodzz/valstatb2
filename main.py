...
# Пример: в ставке написано 04:55, реальное МСК 03:55.
...
def normalize_market(parsed: dict) -> dict:
    ...
    if "фор" in market_norm or "гандикап" in market_norm:
        ...
        if line_value is not None and line_value > 0:
            market_side = "plus"
            market_group = "Фора плюсовая"
        elif line_value is not None and line_value < 0:
            market_side = "minus"
            market_group = "Фора минусовая"
        else:
            market_side = "zero"
            market_group = "Фора 0"
...
def init_db():
    ...
    # Индексы для быстрого боевого скоринга и отчётов.
    db = connect()
...
def make_risk_note(kind: str, existing: dict | None = None, parsed: dict | None = None, extra: dict | None = None) -> str:
    ...
    if kind == "corridor":
        return (
            ...
            f"реком. сумма: {extra.get('stake2')} | в коридоре: {extra.get('corridor_profit')} | вне: {extra.get('outside_profit')} ({extra.get('outside_roi')}%)"
        )
...
def risk_note_to_telegram(risk_status: str, risk_notes: str) -> str:
    ...
    for part in parts[1:]:
        if ":" in part:
            ...
            compact_lines.append(f"<b>{left.strip()}:</b> {right.strip()}")
        else:
            compact_lines.append(part.strip())
...
def analyze_risk(parsed: dict):
    ...
    for row in rows:
        ...
        # 1) Жёсткий повтор: тот же матч + тот же рынок + та же сторона.
        ...
        # 2) Moneyline / победа без ничьи: противоположные стороны могут давать вилку.
        # Пример: Miami Heretics победа @2.70 + FaZe Vegas победа @2.40.
        if (
            same_type
            ...
            risk_kind = "arbitrage" if implied < 1 else "opposite_no_value"
            statuses.append(risk_kind)
            notes.append(make_risk_note(
                risk_kind,
                existing,
                parsed,
                {
        ...
        # 3) Форы / гандикапы: противоположные стороны => коридор или вилка / плечо без плюса.
        if (
            same_type
            ...
            if cond1 and cond2:
                ...
                if lower is not None and upper is not None and lower < upper:
                    ...
                    corridor_profit = round(
                        existing["stake"] * (existing["odds"] - 1)
                        + stake2 * (parsed["odds"] - 1),
                        2,
                    )
        ...
        # 4) Тоталы: ТБ/ТМ => коридор или вилка / плечо без плюса.
        if same_type and parsed.get("market_type") == "total" and existing.get("market_side") != parsed.get("market_side"):
            line1 = existing.get("line_value")
            line2 = parsed.get("line_value")
            if line1 is None or line2 is None:
    ...
    if not statuses:
        ...
        # Если уже есть такая же сторона, это важнее вилки с другой стороной:
        # пользователь не хочет усиливаться/догоняться.
        status = "duplicate"
...
def add_summary_charts(sheet):
    """Добавляет визуальные графики в Summary, если есть нужные таблицы."""
    try:
        # Статусы результата: Win/Lose/Half/Refund/Pending
        labels = [cell.value for cell in sheet["A"]]
        status_names = {"Win", "Lose", "Half win", "Half lose", "Refund", "Pending"}
        status_rows = [i + 1 for i, label in enumerate(labels) if label in status_names]

        if status_rows:
            first = min(status_rows)
            last = max(status_rows)
            pie = PieChart()
            pie.title = "Распределение результатов"
            data = Reference(sheet, min_col=2, min_row=first, max_row=last)
            cats = Reference(sheet, min_col=1, min_row=first, max_row=last)
        ...
        if market_header:
            market_last = market_header
            while market_last + 1 <= sheet.max_row and sheet.cell(row=market_last + 1, column=1).value:
                market_last += 1

            if market_last > market_header:
                bar = BarChart()
                bar.type = "bar"
                bar.style = 10
                bar.title = "Ставки по маркетам"
                bar.y_axis.title = "Маркет"
                bar.x_axis.title = "Количество"
                data = Reference(sheet, min_col=2, min_row=market_header, max_row=market_last)
                cats = Reference(sheet, min_col=1, min_row=market_header + 1, max_row=market_last)
        ...
        if bookmaker_header:
            bookmaker_last = bookmaker_header
            while bookmaker_last + 1 <= sheet.max_row and sheet.cell(row=bookmaker_last + 1, column=1).value:
                bookmaker_last += 1

            if bookmaker_last > bookmaker_header:
                bar2 = BarChart()
                bar2.type = "bar"
                bar2.style = 11
                bar2.title = "Ставки по букмекерам"
                bar2.y_axis.title = "БК"
                bar2.x_axis.title = "Количество"
                data = Reference(sheet, min_col=2, min_row=bookmaker_header, max_row=bookmaker_last)
                cats = Reference(sheet, min_col=1, min_row=bookmaker_header + 1, max_row=bookmaker_last)
...
def risk_decision_kb():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="1000"), KeyboardButton(text="1500"), KeyboardButton(text="2000")],
...
async def add_bet_hint(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Теперь перешли мне сообщение со ставкой.\n"
        "Если формат не подойдёт — я напишу, что именно не смог разобрать.",
        reply_markup=ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text="❌ Отмена")]],
            resize_keyboard=True,
            is_persistent=True,
...
@dp.message(ShiftState.waiting_forwarded_bet)
async def forwarded_bet_handler(message: Message, state: FSMContext):
    text = message.text or message.caption or ""

    if not text.strip():
        await message.answer(
            "⚠️ Я получил сообщение, но в нём нет текста ставки.\n\n"
            "Перешли именно текстовое сообщение со ставкой.",
    ...
    await message.answer(
        ...
        f"{risk_text}\n\n"
        f"<b>Сумма = подтверждение.</b> Выбери кнопку или напиши свою сумму.",
        reply_markup=risk_decision_kb(),
    )


@dp.message(F.text.in_({"💾 Сохранить", "✍️ Другая сумма"}))
...
async def selected_bet_edit_amount_start(message: Message, state: FSMContext):
    ...
    await message.answer("✏️ Напиши новую сумму ставки.", reply_markup=ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="❌ Отмена")]],
        resize_keyboard=True,
        is_persistent=True,
    ))
...
async def save_pending_bet_amount(message: Message, state: FSMContext, amount: float):
    """Сохраняет pending-ставку по сумме. Сумма = подтверждение в быстром режиме."""
    data = await state.get_data()
...
async def show_scored_bet(message: Message, state: FSMContext, parsed: dict):
    """
    Единая точка показа риск-скоринга.
    Работает и после кнопки «Добавить ставку», и при прямом пересыле в бота.
    """
    parsed = analyze_risk(parsed)
...
async def universal_message_handler(message: Message, state: FSMContext):
    ...
    Важная логика:
    1. Ставку можно пересылать напрямую в бота из любого обычного меню.
    2. Кнопка «➕ Добавить ставку» — только подсказка, не обязательное условие.
    3. Если бот ждёт сумму, числовой текст сохраняет ставку.
    4. Если бот ждёт сумму, но вместо суммы пришла новая ставка — бот не молчит,
       а заново проводит риск-скоринг новой ставки.
    5. Если бот ждёт решение по ставке, но пришла новая ставка — старая pending-заявка
       заменяется новой, чтобы бот не зависал в старом состоянии.
    """
    ...
    # Сначала проверяем, не является ли сообщение ставкой.
    # Это нужно ДО блокировки по FSM, чтобы прямой пересыл работал всегда.
    parsed = parse_bet(text)
    ...
    if parsed:
        ...
        # Если была незавершённая pending-ставка и пользователь прислал новую,
        # заменяем её новой без молчания.
    ...
    # Если бот ждёт пересланную ставку, но текст не распознан — даём диагностику.
    ...
    # Если бот ждёт решение по ставке, сумма сразу считается подтверждением.
    # Быстрый режим: переслал → увидел скоринг → нажал/написал сумму → сохранено.
    if current_state == ShiftState.waiting_risk_decision.state:
        try:
            ...
            await message.answer(
                "ℹ️ Выбери сумму кнопкой <b>1000 / 1500 / 2000</b>, напиши свою сумму или нажми <b>🚫 Отказаться</b>.",
                reply_markup=risk_decision_kb(),
            )
            return
    ...
    # Если бот ждёт ручную сумму после кнопки «Другая сумма».
    ...
    # Остальные FSM-состояния специально не трогаем.
    locked_states = {
